...
(() => {
  ...
  const DEFAULT_SETTINGS = {
    ...
    widgets: { wclock: true, wcal: true, wtodo: false, wmovie: true, wweather: false, wcount: false, wpomodoro: false },
  ...
  function bindSiteForm() {
    ...
    // No backdrop-click dismissal here: the form holds typed input (Name/URL), and one stray click
    // outside the card used to throw it all away. Close via ×, Cancel, submit, or Esc instead.
    // ---------- Bulk add panel ----------
  ...
  // The bundled 1.24.2 holiday calendar (assets/sea-holidays.ics, generated by
  // scripts/build-sea-holidays.cjs). Installed as a local:// feed on first launch when the feed
  // list was never customised; deleting it sets lt.caldropped so it is never re-added on this
  // device. A fixed id keeps the colour and the delete-marker stable.
  const SEA_FEED = { id: 'sea-holidays', url: 'local://ics/sea-holidays', asset: 'assets/sea-holidays.ics', fallbackName: '港新马印泰假期' };
  function calDropped() {
    try { return localStorage.getItem('lt.caldropped') === '1'; } catch (_) { return false; }
  }
  // Seed the bundled SEA holiday feed. Only when lt.calendars was never written (null = the
  // default, empty-list state; a present array — even [] — is a user decision we keep). Returns
  // the seeded feed list, or null when seeding did not happen. Failure (file:// preview blocks
  // fetch, corrupted asset) persists nothing, so the next launch retries.
  async function maybeSeedSeaHolidays(rawCals) {
    if (rawCals != null || !window.LT_ICS || !window.LT_CAL) return null;
    if (calDropped()) return null;
    let text = '';
    try {
      const res = await fetch(SEA_FEED.asset);
      if (res && res.ok) text = await res.text();
    } catch (_) {}
    if (!text) return null;
    let raw = [];
    try { raw = window.LT_ICS.parseICS(text); } catch { return null; }
  ...
  function bindCalSettings() {
    ...
    const onListClick = async (e) => {
      ...
      if (e.target.closest('[data-cal-del]')) {
        ...
        if (at < 0) return;
        const removed = state.calendars.splice(at, 1)[0];
        // The bundled holiday feed, once deleted, stays deleted on this device.
        if (removed && removed.url === SEA_FEED.url) {
          try { localStorage.setItem('lt.caldropped', '1'); } catch (_) {}
        }
        delete calCache[id];
        await Store.set(K.calendars, state.calendars);
        await localRawSet(K.calcache, calCache);
  ...
  async function loadDataIntoState() {
    ...
    const rawCals = await localRawGet(K.calendars);
    calCache = (await localRawGet(K.calcache)) || {};
    const seeded = await maybeSeedSeaHolidays(rawCals);
    state.calendars = seeded || normalizeCalendars(rawCals);
