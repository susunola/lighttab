...
- **Free canvas layout** — on wide screens every widget and card can be dragged anywhere; cards snap to a grid and swap places with whatever is already there. The movie card's placement (*Settings → Widgets*) selects the engine: **Left column** tucks it into the icon grid, **Above search** switches to the free canvas where dragging is available
