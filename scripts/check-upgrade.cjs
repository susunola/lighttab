...
page=await open();await page.waitForTimeout(700);const after=await page.evaluate(keys=>chrome.storage.local.get(keys),keys);assert.deepEqual(after,before);assert.equal(await page.evaluate(async()=> (await chrome.storage.local.get('lt.settings'))['lt.settings'].name),'升级用户');assert.equal(await page.evaluate(()=>chrome.runtime.getManifest().version),JSON.parse(fs.readFileSync(path.join(repo,'manifest.json'))).version);assert(await page.locator('#grid').innerText().then(s=>s.includes('升级保留测试')));assert(await page.locator('#todo-list').textContent().then(s=>s.includes('升级后仍保留')));
